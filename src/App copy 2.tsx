...
const App = () => {
  ...
  const [totalMinted, setTotalMinted] = useState(541); // Starting with airdropped
  const [downloadingIds, setDownloadingIds] = useState<Set<string>>(new Set());

  // NFT Tiers Configuration
  ...
  const initializeHashPack = async () => {
    try {
      ...
      console.error('HashPack initialization error:', err);
      setError('Failed to initialize HashPack');
    } finally {
  ...
  const handleMint = async () => {
    ...
    if (wallet.balance < totalCost) {
      setError(`Insufficient balance. You need ${totalCost} HBAR`);
      return;
    }
    ...
    try {
      // Execute transaction through HashPack
      const txResult = await hashpackService.executeMintTransaction(
        wallet.accountId,
        selectedTier,
        quantity,
        totalCost
      ...
      if (txResult.success) {
        // Call backend to finalize mint
        const mintResult = await apiService.mintNFT(
          wallet.accountId,
          quantity
        );

        if (mintResult.success) {
          // Fetch minted NFT metadata
          const nfts = await Promise.all(
            mintResult.result.serials.map((serial: number) =>
              mirrorNodeService.getNFTMetadata(
                process.env.REACT_APP_HEDERA_CONTRACT_ID as string,
                serial
              )
            )
          );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {!wallet.isConnected ? (
              ...
              <div className="bg-gradient-to-br from-amber-900/20 to-transparent border border-amber-500/30 rounded-3xl p-8 backdrop-blur-sm">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <button
                    ...
                    className="group relative overflow-hidden bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-500 hover:to-amber-600 rounded-xl p-6 transition-all duration-300 transform hover:scale-105"
                  >
                  ...
                  <button
                    ...
                    className="group relative overflow-hidden bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 rounded-xl p-6 transition-all duration-300 transform hover:scale-105"
                  >
                    <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div className="relative flex items-center justify-center space-x-3">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="bg-gradient-to-r from-green-900/20 to-transparent border border-green-500/30 rounded-2xl p-4 backdrop-blur-sm flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse" />
                  <div>
                    <div className="text-sm text-gray-400">Connected</div>
                    <div className="font-bold">{wallet.accountId}</div>
                  </div>
                </div>
                  ...
                  {mintedNFTs.map((nft, index) => (
                    <div key={index} className="bg-gradient-to-br from-amber-900/20 to-transparent border border-amber-500/30 rounded-2xl p-4 backdrop-blur-sm">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <a
                          href={`https://hashscan.io/${NETWORK}/token/${nft.tokenId}/${nft.serialNumber}`}
                          target="_blank"
